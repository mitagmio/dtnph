...
from dtb.settings import TELEGRAM_TOKEN_3, DEBUG, TELEGRAM_WEBHOOK_SECRET_3, TELEGRAM_WEBHOOK_URL

from cryptoqueen.handlers.utils import files, error
from cryptoqueen.handlers.onboarding import handlers as onboarding_handlers
from cryptoqueen import tasks as onboarding_handler_task
...
def setup_dispatcher(dp):
    ...
    dp.add_handler(ChatJoinRequestHandler(onboarding_handlers.chatjoinrequesthandler_handler_func))
    dp.add_handler(ChatMemberHandler(onboarding_handlers.chatmemberhandler_handler_func))  
    # handling errors
    ...
    dp.add_handler(MessageHandler(Filters.status_update, onboarding_handlers.status_handler_func))
    return dp


def run_pooling():
    """ Run bot in pooling mode """
    updater = Updater(TELEGRAM_TOKEN_3, use_context=True)

    dp = updater.dispatcher
    dp = setup_dispatcher(dp)

    bot_info = Bot(TELEGRAM_TOKEN_3).get_me()
...
bot = Bot(TELEGRAM_TOKEN_3)
try:
    TELEGRAM_BOT_USERNAME = bot.get_me()["username"]
except telegram.error.Unauthorized:
    logging.error(f"Invalid TELEGRAM_TOKEN_3.")
    sys.exit(1)
...
if TELEGRAM_WEBHOOK_URL == None:
    ...
    url = "%s%s" % (TELEGRAM_WEBHOOK_URL,TELEGRAM_WEBHOOK_SECRET_3)

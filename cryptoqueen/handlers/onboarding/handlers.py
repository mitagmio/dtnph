...
from cryptoqueen.handlers.onboarding import static_text, static_state
from cryptoqueen.handlers.utils.info import extract_user_data_from_update, send_typing_action, is_number
from cryptoqueen.handlers.utils.util import _get_csv_from_qs_values
from cryptoqueen.models import User, Campaign, History
from cryptoqueen.handlers.onboarding.keyboards import *
...
def chatjoinrequesthandler_handler_func(update: Update, context: CallbackContext):
    print('chatjoinrequesthandler_handler_func', update)
...
def chatmemberhandler_handler_func(update: Update, context: CallbackContext):
    print('chatmemberhandler_handler_func', update)
...
def command_start(update: Update, context: CallbackContext):
    ...
    if update.chat_join_request is not None:
        payload = update.chat_join_request.invite_link.invite_link.split('...')[0]
        print(payload)
    if payload is not None and not is_number(payload):
        try:
            camp = Campaign.objects.filter(bot_url__contains=payload).first()
            print(camp.__dict__)
            name = camp.name
            url = camp.target_url
    ...
    if 'photo' in t and t['photo'] != None and len(t['photo']) > 0:
        ...
        id = context.bot.send_photo(
                chat_id=u.user_id, photo=t['photo'][-1]['file_id'], caption=t['caption'] if 'caption' in t else None, caption_entities=entities, reply_markup=make_keyboard_for_start(url = url))
    if 'text' in t and t['text'] != None  and len(t['text']) > 0 and t['text'] != '_':
        ...
        id = context.bot.send_message(
            u.user_id, t['text'], entities=entities, reply_markup=make_keyboard_for_start(url = url), disable_web_page_preview=True)
    if (not 'photo' in t and not 'text' in t) or ('text' in t and t['text'] != None  and len(t['text']) > 0 and t['text'] == '_'):
        id = context.bot.send_message(u.user_id, static_text.START_USER.format(
